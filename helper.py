...
def get_pods_from_namespace(api_instance, namespace="default", labels={}):
    ...
    try:
        pods_list = api_instance.list_namespaced_pod(namespace=namespace).to_dict()[
            "items"
        ]

        if pods_list:
            for pod in pods_list:
                if labels and not compare_labels(labels, pod["metadata"]["labels"]):
                    continue
